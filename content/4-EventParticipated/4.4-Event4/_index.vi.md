...
- DevOps Agent tự động phân loại sự cố, đưa ra giả thuyết, điều tra root cause và đề xuất Mitigation Plan (kế hoạch giảm thiểu lỗi) thay vì tự động thực thi (đảm bảo an toàn).
...
- Amazon Q giúp tự động hóa tạo Job Description (JD), đọc và trích xuất thông tin CV, chấm điểm ứng viên, và tạo báo cáo trực quan mà vẫn giữ tính bảo mật dữ liệu cho doanh nghiệp.

#### Bảo Mật Kết Nối AI và MCP Server (Security)

- Kết nối các AI agents (như Amazon Q) với ứng dụng bên thứ 3 (MCP Server) qua Public Internet rất dễ gặp rủi ro bảo mật (DDoS, Man-in-the-middle).
...
- **Business-first & Thực chiến:** Mọi giải pháp AI (như lọc CV hay hỗ trợ DevOps) đều phải đi từ nỗi đau (pain point) thực tế của doanh nghiệp thay vì chỉ phô diễn công nghệ.
- **Copilot thay vì Autopilot:** AI đóng vai trò như một người trợ lý hỗ trợ (đưa ra mitigation plan, chuẩn bị report), quyền ra quyết định (approval) và thực thi (execute) cuối cùng luôn cần Human-in-the-loop để đảm bảo an toàn, đặc biệt với Production.

#### Kiến Trúc Kỹ Thuật

- **Multi-agent Architecture:** Hiểu được cách phân chia các Agent nhỏ với context window ngắn hơn để giải quyết chuyên biệt từng tác vụ, giúp tối ưu chi phí và kiểm soát phân quyền (RBAC) dễ dàng hơn.
...
- Khi áp dụng AI cho doanh nghiệp, cần đánh giá chi phí (như data transfer in/out, hạ tầng round 53/VPC) để đảm bảo tính khả thi về mặt tài chính.
...
- **Ứng dụng cho phòng ban Non-Tech:** Khảo sát đưa Amazon Q hoặc các công cụ tự động hóa vào team HR, Admin để hỗ trợ đọc tài liệu, screening ứng viên, viết luồng email tự động nhằm tăng năng suất.
...
- Đối với ngôn ngữ có ít tài nguyên như tiếng Việt, việc đi đường vòng (Speech to Text → LLM → Text to Speech) kết hợp với data huấn luyện vùng miền là giải pháp khôn ngoan nhất ở thời điểm hiện tại.
